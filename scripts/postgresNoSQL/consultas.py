...
def snps_in_a_experiment(population_id, reference_id):
    ...
    for chromosome in chromosome_results:
        query_snps = "SELECT DISTINCT v.snp_annotation->>'variation_identification' FROM variation v WHERE v.chrom = '%s' AND v.population_id = '%s'" % (chromosome[0], population_id)
...
def get_all_snps_of_a_chromosome(chromosome_id):
    ...
    cursor = start_up()
    query = " SELECT v.snp_annotation->>'variation_identification' FROM variation v WHERE (v.chrom = %s)" % (chromosome_id)
...
def get_annotations_of_a_population(population_id):
    print("Consulta 3")
    query = "SELECT (v.snp_annotation->>'annotations') FROM variation v WHERE v.snp_annotation ? 'annotations' AND v.population_id = '%s'" % population_id
    cursor = start_up()
...
def variation_related_to_biologic_annotation_and_individuals_related_to_them(biologic_annotation):
    ...
    cursor2 = start_up()
    sum = 0
    query = "SELECT DISTINCT v.snp_annotation->>'variation_identification', v.population_id FROM variation v WHERE v.snp_annotation ->'annotations' ? ' " + biologic_annotation + "'"
    cursor.execute(cursor.mogrify('explain analyze ' + query))
    analyze_result = cursor.fetchall()
    sum += time(analyze_result[-1][0])
...
def get_annotations_related_to_variation(variation_identification):
    print("Consulta 5")
    cursor = start_up()
    query = "SELECT DISTINCT v.snp_annotation->>'annotations' FROM variation v WHERE v.snp_annotation ->'variation_identification' ? '%s'" % variation_identification
    cursor.execute(cursor.mogrify('explain analyse ' + query))
    analyze_result = cursor.fetchall()
    sum = time(analyze_result[-1][0])
...
def get_reference_of_individual(individual_identification):
    print("Consulta 6")
    cursor = start_up()
    query = "SELECT DISTINCT r.reference_file_address FROM reference r, chromosome c, variation v, population p, individual i WHERE i.individual_identification = '%s' AND p.id = i.population_id AND v.population_id = p.id AND c.id = v.chrom AND r.id = c.reference_id" % individual_identification
    cursor.execute(cursor.mogrify('explain analyse ' + query))
    analyze_result = cursor.fetchall()
    sum = time(analyze_result[-1][0])
...
def get_qtd_individuals_with_annotation(annotation):
    print("Consulta 7")
    cursor = start_up()
    query = "SELECT COUNT(DISTINCT(i.id)) FROM individual i, population p, variation v WHERE v.snp_annotation ->'annotations' ?' " + annotation + "' AND v.population_id = p.id AND i.population_id = p.id"
    cursor.execute(cursor.mogrify('explain analyse ' + query))
    analyze_result = cursor.fetchall()
    sum = time(analyze_result[-1][0])
...
def get_individuals_with_annotation(annotation):
    print("Consulta 8")
    cursor = start_up()
    query = "SELECT DISTINCT(i.individual_identification) FROM individual i, population p, variation v WHERE v.snp_annotation ->'annotations' ?' " + annotation + "' AND v.population_id = p.id AND i.population_id = p.id"
    cursor.execute(cursor.mogrify('explain analyse ' + query))
    analyze_result = cursor.fetchall()
    sum = time(analyze_result[-1][0])
...
def snps_of_a_chromosome_in_a_population_at_each_reference(population_id):
    ...
    for reference_id in results_chromosomes.keys():
        ...
        for chromosome in results_chromosomes[reference_id]:
            ...
            query_snps = "SELECT COUNT(v.id) FROM variation v WHERE v.population_id = '%s' AND v.chrom = %s" % (population_id, chromosome[0])
            #cursor.execute(query_snps)
