...
void GenerateInsertGenotype(char individual_id[], char population_id[], char variation_id[], char genotype[], char depth[], FILE *f) {
    char query[default_char_array_size];
...
void GenerateInsertVariations(VCFFields *vcf_fields, IndividualInformation *individual_information, char *population_id, Individual *individuals, int n_of_individuals, FILE *fV, FILE *fG) {
    char query[default_char_array_size];
    ...
    for (i = 0; i < n_of_individuals; i++) {
        ...
        //else
        GenerateInsertGenotype(individuals[i].id, population_id, vcf_fields->id, individual_information[i].genotype, ".", fG);
    }
...
void *ProcessSnp(void *args) {
    ...
    char fGName[default_char_array_size];

    strcpy(fVName, "/Users/carmonia/Documents/output/Variations");
    strcat(fVName, ".csv");

    strcpy(fGName, "/Users/carmonia/Documents/output/Genotype");
    strcat(fGName, ".csv");

    FILE *fV = fopen(fVName, "ab+");
    FILE *fG = fopen(fGName, "ab+");
    ...
    for (int i = 0; i < argument->n_of_lines; i++) {
        GenerateInsertVariations(&vcf_fields[i], individuals_information, argument->population_id, argument->list_of_individuals, argument->n_of_individuals, fV, fG);
...
void ReadFile(char *fileName, VCFMap *vcf_map, Individual **list_of_individuals, char *population_id) {
    ...
    FILE *f = fopen("/Users/carmonia/Documents/output/inserts_individual.csv", "ab+");
...
int main(int argc, char *argv[]) {
    ...
    char file_path[] = {"/Users/carmonia/Documents/HDRA-G6-4-RDP1-RDP2-NIAS.AGCT.vcf"};
    VCFMap vcf_map;
    ...
    FILE *f = fopen("/Users/carmonia/Documents/output/inserts_population.csv", "ab+");
