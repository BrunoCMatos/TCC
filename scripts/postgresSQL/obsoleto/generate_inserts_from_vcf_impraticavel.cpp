...
void GenerateInsertGenotype(int individual_id, int variation_id, char *genotype, FILE *f) {
    char *query = (char*) malloc(sizeof(char) * default_char_array_size);

    strcpy(query, std::to_string(individual_id).c_str());
    strcat(query, ",");
    strcat(query, std::to_string(variation_id).c_str());
    strcat(query, ",");
...
void GenerateInsertVariations(int variation_id, VCFFields *vcf_fields, char *population_id, int n_of_individuals, FILE *fV, Individual *individuals) {
    ...
    strcpy(fGName, "/home/bruno/Documents/Unifesp/ICBD/csv_files/genotype.csv");
    FILE *fG = fopen(fGName, "ab+");

    int i;
    for (i = 0; i < n_of_individuals; i++) {
        GenerateInsertGenotype(individuals[i].id, variation_id, individuals[i].genotype, fG);
    }
...
void ProcessSnp(char **snp_lines, Individual *individuals, int n_of_individuals, char *population_id, VCFMap *vcf_map) {
    ...
    strcpy(fVName, "/home/bruno/Documents/Unifesp/ICBD/csv_files/variation.csv");
