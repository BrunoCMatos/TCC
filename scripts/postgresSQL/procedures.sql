CREATE OR REPLACE FUNCTION function_to_search_3(TEXT, INTEGER) RETURNS INDIVIDUAL[]
/*$1 - variation_identification; $2 - position*/
LANGUAGE plpgsql
AS $$
DECLARE
var_genotypes TEXT[] DEFAULT ARRAY[] :: TEXT[];
var_individual_ids INTEGER[] DEFAULT ARRAY[] :: INTEGER[];
var_reference_allele CHAR;
...
BEGIN
  SELECT v.genotype, v.population_id INTO var_genotypes, var_population_id FROM variation v WHERE v.variation_identification = $1 AND v.pos = $2;

  FOREACH var_genotype IN ARRAY var_genotypes
  LOOP
...
*/
  FOR var_variations IN SELECT DISTINCT v.* FROM variation v, chromosome_variation_annotation cva, chromosome c WHERE c.id = $1 AND v.id = cva.variation_id AND c.id = cva.chromosome_id AND v.population_id = $2
  LOOP
    ...
    var_individual_id := 1;
    FOREACH var_genotype IN ARRAY var_variations.genotype
    LOOP
...
var_individual TEXT;
var_annotation BIOLOGIC_ANNOTATION;
var_variation VARIATION;
BEGIN
    FOR var_variation IN SELECT DISTINCT v.* FROM variation v, biologic_annotation ba, chromosome_variation_annotation cva WHERE ba.id = cva.biologic_annotation_id AND cva.variation_id = v.id AND v.population_id = $1 AND ba.annotation LIKE '%' || $2 || '%'
    LOOP
      var_individual_id := 1;
      /*raise notice 'Variation: %', var_variation.id;*/
      FOREACH var_genotype IN ARRAY var_variation.genotype
      LOOP
        IF substring(var_genotype from 1 for 3) = '0/0' THEN
...
BEGIN
  FOR var_variation IN SELECT DISTINCT v.* FROM variation v, biologic_annotation ba, chromosome_variation_annotation cva WHERE ba.annotation LIKE '%' || $2 || '%' AND  ba.id = cva.biologic_annotation_id AND cva.variation_id = v.id AND v.population_id = $1
  LOOP
    var_individual_id := 1;
    FOREACH var_genotype IN ARRAY var_variation.genotype
    LOOP
  ...
  LOOP
    ...
    /*var_variation_individuals = array_append(var_variation_individuals, var_individual);*/
  END LOOP;
  /*var_variation_individuals = unnest(var_variation_individuals);*/
  RETURN var_individuals;
END
$$;

CREATE OR REPLACE FUNCTION function_to_search_10(INT, INT) RETURNS TEXT[]
...
BEGIN
  FOR var_annotation IN SELECT DISTINCT ba.* FROM biologic_annotation ba, chromosome_variation_annotation cva WHERE ba.id = cva.biologic_annotation_id AND cva.chromosome_id = $2 AND ba.starting_position <= $1 AND ba.ending_position >= $1
  LOOP
    var_annotation_population_array = array_append(var_annotation_population_array, CAST (var_annotation.annotation AS TEXT));
    FOR var_population IN SELECT DISTINCT p.* FROM population p, variation v, chromosome_variation_annotation cva WHERE cva.variation_id = v.id AND cva.biologic_annotation_id = var_annotation.id AND v.population_id = p.id
    LOOP
